...
def noaa_raw_dir(year: int = 2025, datasetid: str = "GHCND") -> Path:
    return RAW_ROOT / "noaa" / datasetid.lower() / str(year)


def noaa_bronze_dir(year: int = 2025, datasetid: str = "GHCND") -> Path:
    return DELTA_ROOT / "bronze" / "noaa" / datasetid.lower() / str(year)


def noaa_silver_dir(year: int = 2025, datasetid: str = "GHCND") -> Path:
